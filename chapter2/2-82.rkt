...
;;2-82 implement the apply-generic to multiple argument and different types at the same time 
;;and test the function with scheme-number-add. However, to achieve the full function, 
;;the package to be install must be modified to be used in the multiple arguments 
;;situation.
;;2-82题实现了能够适用于不同类型多个参数的apply-generic，但是要真正能够对多个参数进行操作，安装的包也需要
;;进行相应的修改，因为此处所用的很多包都仅仅是针对两个参数的。这里为了测试，仅仅修改了有理数包中的加法，实现了
;;有理数连加。
...
;   (hash-table/put (list type1 type2) proc)) 
;  
; (define (ge type1 type2) 
...
(define (install-coercion-package) 
  ...
  (define (scheme-number->rational n) 
    (make-rational (content n) 1)) 
  (put 'scheme-number 'rational scheme-number->rational) 
  (put 'scheme-number 'complex scheme-number->complex) 
  'done) 

(install-coercion-package) 
;(display (add xn yn (make-scheme-number 2)))
(add xc xc)
(add xn xc)
(add xn xn xn)