...
;;racket 也是顺序执行的，在没有载入函数定义前不能具体使用。但没有定义前可以在定义其他函数时调用，只要实际使用时已经定义过了就好。
...
;(display (greatest-common-divisor 4 2))
...
(define (install-rational-package)
  ...
  (define (make-rat n d)
    (if (or (and (integer? n) (integer? d))
            (and (equal? (type-tag n) 'sparse)
                 (equal? (type-tag n) 'sparse)))
...
(define (install-sparse-package);<----------------the sparse package
  ...
  (define (simplify-terms a)
    (define (iter terms gcd-c result)
      (if (null? terms) (reverse result)
          (iter (rest-terms terms) 
                gcd-c 
                (cons (make-term (order (first-term terms))
                                 (/ (coeff (first-term terms)) gcd-c))
                      result))))
  ...
  (define (pseudoremainder-terms t1 t2)
    ...
    (let ((o1 (order (first-term t1)))
      ...
      (iter (mul-terms t1 (list (make-term  0 (expt c (- (+ 1 o1) o2))))) t2 '())))
  ...
  (define (remainder-poly sp1 sp2)
    (if (same-variable? (variable sp1) (variable sp2))
        ...
        (error "Polys not in same var --DIV-POLY"
               (list sp1 sp2))))
  (define (pseudoremainder-poly sp1 sp2)
    (if (same-variable? (variable sp1) (variable sp2))
        (make-poly (variable sp1)
                   (pseudoremainder-terms (term-list sp1)
                                          (term-list sp2)))
        (error "Polys not in same var --DIV-POLY"
               (list sp1 sp2))))
  ...
  (put 'greatest-common-divisor '(sparse sparse)
       (lambda (sp1 sp2) (tag (gcd-poly sp1 sp2))))
  (put 'make '(sparse)
       (lambda (var terms) (tag (make-poly var terms))))
...
(newline)
(newline)
(display "---------2-94gcd-terms--------")
(newline)
(define sp3 (make-sparse 'x '((4 1) (3 -1) (2 -2) (1 2))))

(define sp4 (make-sparse 'x '((3 1) (1 -1))))

(display (greatest-common-divisor sp3 sp4))
(newline)
...
(define p5 (make-sparse 'x '((3 1) (1 -2) (0 1))))

(define p6 (make-sparse 'x '((2 11) (0 7))))

(define p7 (make-sparse 'x '((1 13) (0 5))))
