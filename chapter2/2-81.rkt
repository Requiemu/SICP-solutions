...
;coercion:implement the operation that can handle the mix of scheme-number and the complex number.
...
;a.
;(apply-generic 'exp xc yc). This can't be evalued because there is #f in the 'exp '(complex complex)
;in the table and the 'complex type will be coerced to itself to no end.

;b.For the current apply-generic procedure there is no need for this because the precedure for the 
;the same type argument the proc is already exist.
;    (let ((proc (get op type-tags)))<-------------here the procedure for the same type is handled
;      (if proc
;          (apply proc (map content args))

;c.Just need to add a judgement statement to the apply-generic
;
;(define (apply-generic op . args)
;  (let ((type-tags (map type-tag args)))
;    (let ((proc (get op type-tags)))
...
;                    (a1 (car args))
;                    (a2 (cadr args)))
;                (if (eq? type1 type2) <----------------------the judgement statement
;                    (error "There is no produre for" op type1 type2)
;                    (let ((t1->t2 (get type1 type2))
;                          (t2->t1 (get type2 type1)))
;                      (cond (t1->t2
;                             (apply-generic op (t1->t2 a1) a2))
