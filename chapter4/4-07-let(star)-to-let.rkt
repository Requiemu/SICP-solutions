...
(define (let*->nested-lets exp)
  (define (iter pairs)
    (if (null? pairs)
        (let*-body exp)
        (make-let 
         (list (first-pair pairs))
         (iter (rest-pairs pairs)))))
  (iter (let*-pairs exp)))

(define test-let* (list 'let* '((x 2) (y (+ x 2))) '(+ x y)))

(let*->nested-lets test-let*)

;;add (eval (let*->nested-lets exp) env) to the eval will work because the "eval" has already know how to handle nested-lets
