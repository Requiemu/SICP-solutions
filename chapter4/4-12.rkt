...
(define (lookup-variable-value var env)
  (define (env-loop sub-env)
    (define (scan frame)
      (cond ((null? frame)
             (env-loop (enclosing-environment sub-env)))
            ((eq? var (pair-var (first-pair frame)))
             (pair-val (first-pair frame)))
            (else (scan (rest-pairs frame)))))
    (if (eq? sub-env the-empty-environment)
        (error "Unbound variable" var)
        (let ((frame (first-frame sub-env)))
          (scan frame))))
  (env-loop env))
  ...
  (define (env-loop env)
    (define (scan frame)
      (cond ((eq? frame the-empty-frame)
             (env-loop (enclosing-environment env)))
            ((eq? var (pair-var (first-pair frame)))
             (set-pair-value! (first-pair frame) val))
            (else (scan (rest-pairs frame)))))
    (if (eq? env the-empty-evironment)
  ...
  (let ((frame (first-frame env)))
    (define (scan frame)
      (cond ((eq? frame the-empty-frame)
             (add-binding-to-frame! var val frame))
            ((eq? var (pair-var (first-pair frame)))
             (set-pair-value! (first-pair frame)))
            (else (scan (rest-pairs frame)))))
    (scan frame)))
