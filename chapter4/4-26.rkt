...
;;add the normal evaluation edition of the syntax unless.
(define (analyze exp)
  (cond ((self-evaluating? exp)
        ...
        ((unless? exp) (analyze-if (unless->if exp)))
...
(define (pairs-var pairs) (map car pairs))
(define (pairs-val pairs) (map cadr pairs))

(define (let->lambda-application exp)
  (cons (make-lambda (pairs-var (let-pairs exp)) 
                     (let-body exp))
        (pairs-val (let-pairs exp))))
(define (analyze-let exp)
  (analyze-application (let->lambda-application exp)))



(define (unless? exp) (tagged-list? exp 'unless))
(define (unless-predicate exp) (cadr exp))
(define (unless-consquent exp) (caddr exp))
(define (unless-alternate exp) (cadddr exp))

(define (unless->if exp)
  (make-if (unless-predicate exp)
           (unless-alternate exp)
           (unless-consquent exp)))
...
;;if "if" and "unless" is a procedure, following can be implement, while if "if" and "unless" 
;;is just a syntax, following can't be implemented.

;;(f if) will produce 1

;;(f unless) will produce 0
